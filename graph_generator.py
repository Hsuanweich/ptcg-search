...
def get_q1q2q3(search_key, from_where):
    ...
    with connection.cursor(dictionary=True) as cursor:
        cursor.execute("SELECT `price_date`, `q1`, `median`, `q3` FROM `product_week` "
                       "WHERE `search_key` = %s AND `from_where` = %s;", (search_key, from_where))
    ...
    return price_date, q1, median, q3
...
def cal_recommended_price(labels, values):
    recommended_price = 0
    value_num = 0
    i = 0
    for label in labels:
        value_num += values[i]
        price = int(label.split('~')[0]) + 4
