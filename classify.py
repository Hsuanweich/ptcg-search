...
def classifymain():
    ...
    num_classes = 3  # 假設有3個類別，根據實際情況修改
    mobilenet_v2.classifier[1] = torch.nn.Linear(mobilenet_v2.last_channel, num_classes)


    # 加載訓練過的模型權重
    mobilenet_v2.load_state_dict(torch.load("/home/ptcg/PTCGHTML/ptcg_cards/classify/mobilenet_v2_finetuned_m3c.pth", map_location=torch.device('cpu')))
    """
    ***************************************************************************
